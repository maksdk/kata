...
export class Ch14Particles {
    ...
    static async CreateParticles() {
        ...
        val = $('#id-mass').val();
        Ch14Particles.massScale = parseFloat(val as string);
        val = $('#id-color').val();
        Ch14Particles.colorOpacity = parseFloat(val as string);

        const count:number = 1000;
        const size:vec3 = [8,5,5];
        const len = count*4;

        const particleData = new Float32Array(len);
        for(let i=0; i<len; i += 4){
            particleData[i] = Math.random()*size[0] - size[0]/2;
            particleData[i+1] = Math.random()*size[1] - size[1]/2;
            particleData[i+2] = Math.random()*size[2] - size[2]/2;
            particleData[i+3] =1;
        }
        const particleBuffer = T3D.CreateGPUBuffer(device, particleData, GPUBufferUsage.VERTEX | GPUBufferUsage.STORAGE);
        ...
        const colorData = new Float32Array(this.numParticles);
        ...
        const computeData = new Float32Array([
            ...
            Math.random() * this.massScale/this.numParticles, 
            Math.random() * this.massScale/this.numParticles, 
            Math.random() * this.massScale/this.numParticles, 0 // Mass factors
        ]);
        ...
        // compute pipeline
        //const shaders = Ch14Shaders.wgslShaders1(numParticles);
        const shaders = Ch14Shaders.glslShaders1(this.numParticles);
        const computePipeline = device.createComputePipeline({
            layout: device.createPipelineLayout({ bindGroupLayouts: [computeBindGroupLayout] }),
            computeStage: {
                module: device.createShaderModule({
                    //code: shaders.compute
                    code: glslang.compileGLSL(shaders.compute, 'compute')
                }),
                entryPoint: "main"
            }
        });
        ...
        function draw() {
            ...
            renderPassDescriptor.colorAttachments[0].attachment = gpu.swapChain.getCurrentTexture().createView();
